...
export const storageService = {
  ...
  async addPrompt(prompt: Omit<Prompt, 'id'>): Promise<string> {
    ...
    if (useLocalStorage) {
      addLocalPrompt({ ...prompt, id } as Prompt);
      return id;
    }
    try {
      return await promptService.add({ ...prompt, id });
    } catch (error) {
      console.warn('Firebase failed, falling back to localStorage:', error);
      addLocalPrompt({ ...prompt, id } as Prompt);
      return id;
  ...
  async addProject(project: Omit<Project, 'id'>): Promise<string> {
    const id = crypto.randomUUID();
    if (useLocalStorage) {
      addLocalProject({ ...project, id } as Project);
      return id;
    }
    try {
      return await projectService.add({ ...project, id });
    } catch (error) {
      console.warn('Firebase failed, falling back to localStorage:', error);
      addLocalProject({ ...project, id } as Project);
      return id;
  ...
  async addCategory(category: Omit<Category, 'id'>): Promise<string> {
    const id = crypto.randomUUID();
    if (useLocalStorage) {
      addLocalCategory({ ...category, id } as Category);
      return id;
    }
    try {
      return await categoryService.add({ ...category, id });
    } catch (error) {
      console.warn('Firebase failed, falling back to localStorage:', error);
      addLocalCategory({ ...category, id } as Category);
      return id;
